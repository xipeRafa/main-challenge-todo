...
const api = {
    ...
    data: [
           ...
           {id:Date.now()+3, isActive:true, content:"cccccc"}],

    dataEdit:[{content:"edit mode"}],
    ...
    /*   let x = document.getElementsByClassName("disabled");
      ...
      this.dataEdit.splice(0,1,{id:idElementClicked, index:ind, content:''})  
    ...
    insert: function(){
      ...
      let indexClicked = this.dataEdit.slice(0)[0].index //index of element clicked

      let lengthClicked = this.dataEdit.slice(0)[0].content.length  //last element clicked (length)

      let idClicked = this.dataEdit.slice(0)[0].id //id of element clicked
      ...
      if(lengthClicked===0){
        ...
        this.dataEdit.splice(0,1,{content:'edit mode again'})
...
class App {
  ...
  render() {
      ...
      let listItems = " "
       let l = this.data.map(el => el.content === 'To do List Ready to Use!!!' ? 'verr' : 'flases')[0]
      ...
      while (count--) {
        listItems += `
          ...
          <button class="delet disabled id="${l}" " onClick="todoApp.delet()">Delete</button>
